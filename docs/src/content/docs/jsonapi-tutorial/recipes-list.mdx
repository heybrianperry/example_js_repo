...
<Aside>
  ...
  JSON:API
  Client](http://localhost:4321/getting-started/quick-start/#install-using-a-package-manager)
...
<Tabs>
  ...
  <TabItem label="Data">
    <JsonView client:only collapsed={2} src={recipes} />
  </TabItem>
</Tabs>
...
<JsonView client:only collapsed={2} src={recipe} />
